...
project_gid = constants.ASANA_SALES_PROJECT_ID


sales_task = [] #Lista vacia inicial
try:
    # Traer tareas del proyecto
    api_response = tasks_api_instance.get_tasks_for_project(project_gid, constants.OPTS)
    for data in api_response:
      sales_task.append(data)
    print('--'*20)    
    print("data cargada API: ", len(sales_task))
    print('--'*20)
except ApiException as e:
    print('--'*20)
...
gid_pais = constants.ASANA_CUSTOM_FIELD_COUNTRY_ID
gid_fecha_venta = constants.ASANA_CUSTOM_FIELD_FECHA_VENTA_ID
gid_razon_perdida = constants.ASANA_CUSTOM_FIELD_RAZON_PERDIDA_ID


sales_task_clean = []  # Lista limpia para construir info final

for i in sales_task:
    # Construir el diccionario final de la tarea
    sales_task_clean.append({
        'gid': i['gid'],
        ...
        'country': next((field.get('display_value') for field in i.get('custom_fields', []) if field.get('gid') == gid_pais), None),
        'date_sale': next((field.get('display_value') for field in i.get('custom_fields', []) if field.get('gid') == gid_fecha_venta), None),
...
sales_task_df = pd.DataFrame(sales_task_clean)
print('--'*20)
print('longitud dataframe: ',len(sales_task_df ))
print('--'*20)
print(sales_task_df.head())


# ## 5. Data typing
## Tipado de datos
# Definir el tipo de dato correspodiente a cada columna
sales_task_df['gid'] = sales_task_df['gid'].astype(int)
sales_task_df['project_id'] = sales_task_df['project_id'].astype(int)
sales_task_df['section_id'] = sales_task_df['section_id'].fillna(0).astype(int)
sales_task_df['created_at'] = pd.to_datetime(sales_task_df['created_at'], utc=True)
sales_task_df['assigne_id'] = sales_task_df['assigne_id'].fillna(0).astype(int)
sales_task_df['completed_at'] = pd.to_datetime(sales_task_df['completed_at'], utc=True)
sales_task_df['actual_time_minutes'] = sales_task_df['actual_time_minutes'].fillna(0).astype(int)
sales_task_df['modified_at'] = pd.to_datetime(sales_task_df['modified_at'], utc=True)


## limpieza de datos
# Recorte de espacios en blanco
sales_task_df["name"] = sales_task_df["name"].str.strip()
sales_task_df["date_sale"] = sales_task_df["date_sale"].str.strip()
sales_task_df["restaurant"] = sales_task_df["restaurant"].str.strip()
# Poner en miniscula
sales_task_df["restaurant"] = sales_task_df["restaurant"].str.lower()
...
cols = ["gid","pipedrive_id","resource_subtype", "name", "project_id",
        ...
        "id", "restaurant", "country", "date_sale","lost_reason",
        "completed", "completed_at","actual_time_minutes", "modified_at", "permalink_url"]
sales_task_df = sales_task_df[[c for c in cols if c in sales_task_df.columns]]

print('--'*20)
print('tipado de datos: ')
print('--'*20)
print(sales_task_df.dtypes)
print('--'*20)
...
'''
# ## 11. Final Table
sales_task_df = sales_task_df.drop_duplicates().reset_index(drop=True)
print('--'*20)
print(f"Longitud sales_task_df a cargar: {len(sales_task_df)}")
print('--'*20)
